...
nodes = read.delim('/home/soyeon1771/SNFtools/SNFtools/inters.txt', sep=" ", header=F)
trues = as.numeric(read.delim('/home/soyeon1771/SNFtools/SNFtools/trues.txt', sep=" ", header=F))

# read text graph
# The edges should be distances (diag = 0)
text_fname_list = c("adj_socialtfidf_jaccardvit_1558.txt", 
               "adj_word2vec_vflickr_1558.txt", 
               "adj_word2vec_venwiki_1558.txt")

img_fname_list = c("adj_HOG_cosine_1558.txt",
               "adj_VGG_cosine_1558.txt",
               "adj_Res152-Caffe_cosine_1558.txt")
               #"adj_LeNet_cosine_1558.txt")
...
text_scores = read.delim('/home/soyeon1771/SNFtools/SNFtools/baseSC/result_txt_singlebaseSC_gkernel_1558.txt', sep="\t", header=F, 
                    col.names=c('type','alpha','ARI','NMI'))

image_scores = read.delim('/home/soyeon1771/SNFtools/SNFtools/baseSC/result_img_singlebaseSC_gkernel_1558.txt', sep="\t", header=F, 
                         col.names=c('type','alpha','ARI','NMI'))
...
single_text_ARI = text_scores[text_scores$alpha==0.3, 'ARI'][1]
single_text_NMI = text_scores[text_scores$alpha==0.3, 'NMI'][1]
...
baseSC = read.delim('/home/soyeon1771/SNFtools/SNFtools/baseSC/result_multibaseSC_gkernel_1558.txt', sep="\t", header=F, 
...
bar1 <- ggplot(data=maxdf, aes(x=reorder(type, ARI), y=ARI, fill=type)) + 
  ...
  theme(axis.text.x=element_text(angle=40, hjust=1, size=rel(1.7)),
...
# nodes
single_text_nodes = data.frame(node=unlist(nodes), label=t(Wt_label))
single_img_nodes = data.frame(node=unlist(nodes), label=t(Wi_label))
fused_nodes = data.frame(node=unlist(nodes), label=res_fused_NMI[[1]])

write.table(single_text_nodes, file='single_text_nodes.csv',row.names = F,col.names = F)
write.table(single_img_nodes, file='single_img_nodes.csv',row.names = F,col.names = F)
write.table(fused_nodes, file='fused_nodes.csv',row.names = F,col.names = F)

# edges
colnames(W_text[[text_gname_list[1]]]) <- nodes
rownames(W_text[[text_gname_list[1]]]) <- nodes

colnames(W_img[[img_gname_list[3]]]) <- nodes
rownames(W_img[[img_gname_list[3]]]) <- nodes

colnames(W_fused) <- nodes
rownames(W_fused) <- nodes

single_text_edges = melt(W_text[[text_gname_list[1]]])
single_img_edges = melt(W_img[[img_gname_list[3]]])
fused_edges = melt(W_fused)

write.table(single_text_edges, file='single_text_edges.csv',row.names = F,col.names = F)
write.table(single_img_edges, file='single_img_edges.csv',row.names = F,col.names = F)
write.table(fused_edges, file='fused_edges.csv',row.names = F,col.names = F)
