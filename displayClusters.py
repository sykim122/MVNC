...
def spectralClustering(W, type=2, K=10):
    ...
    #DL[DL<0.001] = 0
    DLs = scipy.sparse.csc_matrix(DL).astype('float64')
    value, vector = scipy.sparse.linalg.eigs(DLs, K, sigma=0)    # matrix consistance
    value = value.real
    vector = np.asmatrix(vector.real)
    U = vector[:, np.argsort(value)[:K]]

    eigDiscrete = discresation(U, K)

    labels = np.array(np.where(eigDiscrete == np.max(eigDiscrete))[1])
